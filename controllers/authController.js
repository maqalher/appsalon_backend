...
const register = async(req, res) => {
    ...
    const MIN_PASSWPRD_LENGTH = 8
    if(password.trim().length < MIN_PASSWPRD_LENGTH){
        const error = new Error(`El password debe de contener${MIN_PASSWPRD_LENGTH} caracteres`)
...
const verifyAccount = async(req, res) => {
    ...
    try {
        ...
        user.token = ""
        await user.save()
        res.json({msg: 'Usuario Confirmado Correctamante'})
    } catch (error) {
        console.log(error);
    }
}
...
const login = async(req, res) => {
    ...
    if(!user.verified){
        const error = new Error('Tu cuenta no ha sido confimada aun')
        return res.status(401).json({msg: error.message})
    }
